...
- Added two new AI pilot policy resources derived from the user-policy source document: one guide on access/approval/revocation controls and one template-style article on data boundaries, human review, and value tracking. Both were written without source company or individual names in the article content and were registered in `assets/data/resources-data.js`, `insights.html`, and `sitemap.xml` with matching SVG cover assets.
- Added two additional AI pilot resources derived from the POC source document: one guide focused on intake and use-case selection quality, and one template-style article focused on midpoint reviews, ROI, and go/no-go decisions. These were also written without source company or individual names in the article content and were registered in `assets/data/resources-data.js`, `insights.html`, and `sitemap.xml` with matching SVG cover assets.
- Added a new AI strategy article from supplied copy on when to use a secondary AI tool instead of the default workplace assistant, and registered it in `assets/data/resources-data.js`, `insights.html`, and `sitemap.xml` with a matching SVG cover asset.
- Added a new AI governance article from supplied copy on pilot-program guardrails, including acceptable use, data classification, human review, monitoring, and compliance, and registered it in `assets/data/resources-data.js`, `insights.html`, and `sitemap.xml` with a matching SVG cover asset.
- Added a new AI pilot rollout article from supplied copy on structuring pilots with clear business goals, tight scope, measurable ROI, participant selection, and governance from day one, and registered it in `assets/data/resources-data.js`, `insights.html`, and `sitemap.xml` with a matching SVG cover asset.
